...
class InstallerTests(unittest.TestCase):
 ...
 def test_install_validate_uninstall_preserves_existing_patch_and_package(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   self.assertEqual(original,patch.read_text(encoding='utf-8')); self.assertEqual('preimage',(pre/'old.txt').read_text()); self.assertEqual('state-preimage',(old_state/'old-state.txt').read_text()); self.assertFalse((home/'.graphlean-installer-v1.0.0').exists())
 def test_modified_install_aborts_without_partial_uninstall_then_explicit_restore(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   r=self.runpy(ROOT/'UNINSTALL.py','--dsh-home',home); self.assertEqual(2,r.returncode); self.assertTrue(p.exists()); self.assertEqual(patch_before,(home/'cordis.patch.yml').read_text()); self.assertTrue((home/'.graphlean-installer-v1.0.0/install.json').exists())
 ...
 def test_manifest_path_injection_is_rejected_before_destructive_action(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   manifest=home/'.graphlean-installer-v1.0.0/install.json'; d=json.loads(manifest.read_text()); d['packagePath']=str(sentinel); manifest.write_text(json.dumps(d))
 ...
 def test_noncanonical_semantically_empty_patch_is_rejected_before_mutation(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   pre=home/'graphlean/plugin'; pre.mkdir(parents=True); (pre/'old.txt').write_text('unchanged')
   r=self.runpy(ROOT/'INSTALL.py','--dsh-home',home); self.assertEqual(2,r.returncode); self.assertEqual(original,patch.read_text()); self.assertEqual('unchanged',(pre/'old.txt').read_text()); self.assertFalse((home/'.graphlean-installer-v1.0.0').exists())
 def test_preexisting_unmarked_graphlean_patch_entry_is_rejected(self):
  with tempfile.TemporaryDirectory() as td:
   home=Path(td); patch=home/'cordis.patch.yml'; original="- insert:\n    - id: graphlean-gate\n      name: './other.js'\n"; patch.write_text(original)
   r=self.runpy(ROOT/'INSTALL.py','--dsh-home',home); self.assertEqual(2,r.returncode); self.assertEqual(original,patch.read_text()); self.assertFalse((home/'.graphlean-installer-v1.0.0').exists())
 ...
 def test_corrupted_bound_backup_is_rejected_before_uninstall(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   r=self.runpy(ROOT/'INSTALL.py','--dsh-home',home); self.assertEqual(0,r.returncode,r.stderr)
   manifest=json.loads((home/'.graphlean-installer-v1.0.0/install.json').read_text()); backup=home/'.graphlean-installer-v1.0.0/backups'/manifest['backupId']/'plugin-preimage'/'old.txt'; backup.write_text('CORRUPTED')
   ...
   r=self.runpy(ROOT/'UNINSTALL.py','--dsh-home',home,'--restore-backup'); self.assertEqual(2,r.returncode); self.assertEqual(before,current.read_bytes()); self.assertTrue((home/'.graphlean-installer-v1.0.0/install.json').exists())
...
class InstallerFaultInjectionTests(unittest.TestCase):
 ...
 def test_staging_commit_failure_restores_preimages(self):
  ...
  with tempfile.TemporaryDirectory() as td:
   ...
   self.assertEqual('old-plugin',(plugin/'old.txt').read_text()); self.assertEqual('old-state',(state/'old.txt').read_text()); self.assertEqual(original,patch_file.read_text()); self.assertFalse((home/'.graphlean-installer-v1.0.0').exists())
 def test_backup_failure_leaves_host_and_no_metadata(self):
  ...
  with tempfile.TemporaryDirectory() as td:
   ...
   with patch.object(mod,'copytree',side_effect=fail_backup):
    with self.assertRaises(OSError): mod.main(['--dsh-home',str(home)])
   self.assertEqual('old-plugin',(plugin/'old.txt').read_text()); self.assertFalse((home/'.graphlean-installer-v1.0.0').exists())
...
class InstallerSymlinkSafetyTests(unittest.TestCase):
 ...
 def test_uninstall_detects_nested_plugin_symlink_even_with_matching_content(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   r=self.runpy(ROOT/'UNINSTALL.py','--dsh-home',home); self.assertEqual(2,r.returncode); self.assertTrue(index.is_symlink()); self.assertTrue((home/'.graphlean-installer-v1.0.0/install.json').exists())
...
class OneClickReleaseTests(unittest.TestCase):
 ...
 def test_unix_one_click_round_trip(self):
  ...
  with tempfile.TemporaryDirectory() as td:
   ...
   self.assertEqual(original,pre.read_bytes()); self.assertFalse((home/'.graphlean-installer-v1.0.0').exists())
 def test_auto_real_loader_probe_success_and_failure_rollback(self):
  ...
  with tempfile.TemporaryDirectory() as td:
   ...
   r=self.runpy(ROOT/'INSTALL.py','--dsh-home',bad,'--probe-dsh','auto',env=env); self.assertNotEqual(0,r.returncode); self.assertEqual(original,patch.read_bytes()); self.assertFalse((bad/'graphlean/plugin').exists()); self.assertFalse((bad/'.graphlean-installer-v1.0.0').exists())
