...
class EvolutionTests(unittest.TestCase):
 def test_evolution_is_graph_and_approval_is_session_bound(self):
  ...
  with tempfile.TemporaryDirectory() as td:
   ...
   p=graphleanctl.approve(root,run,h); token=json.loads(p.read_text()); self.assertEqual(h,token['candidateHash']); self.assertEqual(3,token['schemaVersion']); self.assertEqual(session,p.parent)
 ...
 def test_out_of_band_recover_abort_archives_pending_run_without_replay(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   self.assertFalse(active.exists()); self.assertEqual(session/f'aborted-{run}.json',archived); self.assertTrue(archived.is_file()); self.assertFalse(review.exists()); self.assertFalse(approval.exists())
